...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setSuccessMessage(<img src={Loader} style={{width:"3vw"}}/>)
    try {
      ...
      if (response.data.token) {
        ...
        if(responseToken.data.enabledtwofactorauth===true){
          ...
          // let encryptedQRdata = CryptoJS.AES.encrypt(responseToken.data.otpauth_url,"key")
  ...
  const handleSubmit2FACode = () =>{
    seterr2FA(<img src={WhiteLoader} style={{width:"3vw"}}/>)
   
   if(TFACode.length>0){
   let token = TFACode
   let secret =  localStorage.getItem('cloudsecret');
   axios.post("https://services.uaxwallet.com/api/verifyTwoFA",{token:token,secret:secret})
   .then(res=>{
